    ...
    <div class="flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
        ...
        <div class="w-full max-w-4xl p-8 sm:p-10 space-y-8 bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/50 fade-in relative z-10">
            ...
            <div class="text-center">
                ...
                <h1 class="text-3xl font-extrabold text-slate-900 bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent">
                    Form Pendaftaran Member Baru
                </h1>
                <p class="mt-3 text-lg text-slate-600 max-w-2xl mx-auto">
                    Bergabunglah dengan keluarga besar Cikampek Swimming Club dan mulailah petualangan renang Anda!
                </p>
            </div>
            ...
            <form class="mt-8 space-y-10" action="{{ route('member.register.create') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input id="name" name="name" type="text" required
                                   class="w-full px-4 py-3 border border-slate-300 rounded-xl shadow-sm focus:ring-2 focus:ring-cyan-500 bg-white/80 backdrop-blur-sm"
                        ...
                        <div>
                            ...
                            <select id="gender" name="gender" required
                                    class="w-full px-4 py-3 bg-white/80 backdrop-blur-sm border border-slate-300 rounded-xl shadow-sm focus:ring-2 focus:ring-cyan-500">
                ...
                <fieldset class="p-6 bg-gradient-to-br from-sky-50 to-cyan-50 rounded-2xl border border-cyan-100">
                    ...
                    <div class="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                        ...
                        <div class="space-y-6">
                            <div>
                                <label for="training_package_id" class="block text-sm font-semibold text-slate-700 mb-2">
                                    Pilih Paket Latihan
                                </label>
                                ...
                                >
                                    ...
                                    @foreach ($trainingPackages as $package)
                                        <option value="{{ $package->id }}" data-price="{{ $package->price }}"
                                            {{ old('training_package_id')==$package->id?'selected':'' }}>
                            ...
                            <div class="bg-gradient-to-br from-white to-slate-50 p-6 rounded-xl border border-slate-200 text-center shadow-sm">
                                ...
                                <p id="total_price" class="text-4xl font-extrabold bg-gradient-to-r from-cyan-500 to-blue-600 bg-clip-text text-transparent mt-2">
                                    Rp 0
                                </p>
                                <p class="text-xs text-slate-500 mt-2">
                                    *Sudah termasuk biaya pendaftaran dan paket latihan
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="p-6 border border-slate-200 rounded-xl bg-gradient-to-br from-white to-slate-50 shadow-sm">
                                ...
                                <label class="block text-sm font-semibold text-slate-700 mb-2">Pilih Rekening Tujuan</label>

                                <select id="bank_account_id" name="bank_account_id" required
                                        class="w-full px-4 py-3 bg-white/80 backdrop-blur-sm border border-slate-300 rounded-xl shadow-sm focus:ring-2 focus:ring-cyan-500"
                                        data-accounts='@json($bankAccounts)'>
                                ...
                                <div id="selected_bank_details" class="mt-4"></div>
                            </div>

                            <!-- File Upload -->
                            <div x-data="{ fileName: '' }" class="space-y-3">
                                <label for="proof_of_payment" class="block text-sm font-semibold text-slate-700">
                                    Upload Bukti Transfer
                                </label>

                                <div class="mt-2 flex justify-center rounded-xl border-2 border-dashed border-slate-300 px-6 py-10 hover:border-cyan-400 hover:bg-cyan-50/50">
                                    <div class="text-center">
                                        <svg class="mx-auto h-12 w-12 text-slate-400" viewBox="0 0 24 24" fill="currentColor">
                                        ...
                                        <div class="mt-4 flex text-sm justify-center text-slate-600">
                                            <label class="relative cursor-pointer rounded-md bg-white font-semibold text-cyan-600 hover:text-cyan-500">
                                        ...
                                        <p x-show="fileName"
                                           x-text="'File terpilih: ' + fileName"
                                           class="text-sm font-semibold text-green-600 mt-3 bg-green-50 px-3 py-2 rounded-lg">
                                        </p>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="pt-6">
                    <button type="submit"
                            class="group relative flex w-full justify-center rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 py-4 px-6 text-lg font-bold text-white hover:from-cyan-600 hover:to-blue-700 shadow-lg hover:shadow-xl transition-all">
                        <svg class="h-6 w-6 mr-3 group-hover:scale-110 transition-transform"
                    ...
                    <p class="text-center text-sm text-slate-500 mt-4">
                        ...
                        <a href="#" class="text-cyan-600 font-medium underline">syarat dan ketentuan</a>.
                    </p>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const display = document.getElementById('selected_bank_details');
        const accounts = JSON.parse(bankSelect.dataset.accounts || '[]');

        function updateBank() {
            const acc = accounts.find(a => a.id == bankSelect.value);

            display.innerHTML = acc ? `
                <div class="p-4 bg-white border border-slate-200 rounded-lg shadow-sm space-y-2">
                    <div>
                        <span class="font-bold text-slate-800">${acc.bank_name}</span>
                        <p class="text-xs text-slate-500 mt-1">a.n. ${acc.account_holder}</p>
                    </div>

                    <div class="flex items-center justify-between bg-slate-50 p-2 rounded-md">
                        <span class="text-lg font-mono text-cyan-700">${acc.account_number}</span>

                        <button type="button" onclick="copyAccountNumber(this)"
                                class="px-3 py-1 text-xs font-semibold text-cyan-700 bg-cyan-100 rounded-md">
                            Salin
                        </button>
                    </div>
                </div>
        ...
        bankSelect.addEventListener('change', updateBank);
        updateBank();
    });
</script>
